...
from AISversion import annealed_importance_sampling as AISOLD
...
rbm1 = MODEL.BinaryBinaryRBM(number_visibles = 28*28, 
                                   number_hiddens = 16, 
                                   data=None, 
                                   initial_weights=0.0, 
...
b = b[0:b.shape[0]-1]
c = numx.linspace(0.0, 1.0,10000+1)
print c
betas = numx.hstack((a,b,c))
#
numx.random.seed(42)
logZ = AISOLD(rbm, num_runs=100, beta=c,data = None)[0][0]
...
#print 'AIS min LL', numx.mean(ESTIMATOR.log_likelihood_v(rbm, logZ, train_data))
numx.random.seed(42)
logZ, logSTDup,logSTDdown = AISOLD(rbm, num_runs=100, beta=c,data = train_data       )
print 'AIS min LL', numx.mean(ESTIMATOR.log_likelihood_v(rbm, logZ, train_data))
print logZ,'\t',logSTDup, '\t ',logSTDdown
