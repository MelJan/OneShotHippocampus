...
def getbasebias(data=None):
    ...
    if data != None:
        p_int = (data.sum(axis=0).reshape(1,data.shape[1])+1.0)/(data.shape[0]+1.0)
        #p_int = numx.clip(data.mean(axis=0), 1.0/300000.0, 1.0-1.0/300000.0).reshape(1,data.shape[1])
...
def annealed_importance_sampling(rbm, 
    ...
    for bb in beta[1:beta.shape[0]-1]:  # muss bis 1.0 gehen bei ruslan nicht korrekt
