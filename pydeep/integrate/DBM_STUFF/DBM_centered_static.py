...
b1 += numx.dot(0.0-o2,W1.T)
b2 += numx.dot(0.0-o1,W1) + numx.dot(0.0-o3,W2.T)
b3 += numx.dot(0.0-o2,W2)
...
for epoch in range(0,epochs) :
    for b in range(0,data.shape[0],batch_size):
        ...
        db1 = numx.mean(d1-m1,axis = 0).reshape(1,N)-numx.dot(o2,dW1.T)
        db2 = numx.mean(d2-m2,axis = 0).reshape(1,M)-numx.dot(o1,dW1) -numx.dot(o3,dW2.T)
        db3 = numx.mean(d3-m3,axis = 0).reshape(1,O)-numx.dot(o2,dW2)
        ...
        print numx.mean(numxExt.get_norms(b1)),'\t',numx.mean(numxExt.get_norms(b2)),'\t',
        print numx.mean(numxExt.get_norms(b3)),'\t',numx.mean(o1),'\t',numx.mean(o2),'\t',numx.mean(o3)
...
m2 = Sigmoid.f(numx.dot(d1,W1) + numx.dot(d3,W2.T) + b2)
m2 = numx.float64(m2 > numx.random.random(m2.shape))
m1 = Sigmoid.f(numx.dot(m2,W1.T) + b1)
...
m3 = Sigmoid.f(numx.dot(m2,W2) + b3)
m3 = numx.float64(m3 > numx.random.random(m3.shape))
m2 = Sigmoid.f(numx.dot(m1,W1) + numx.dot(d3,W2.T) + b2)
m2 = numx.float64(m2 > numx.random.random(m2.shape))
m1 = Sigmoid.f(numx.dot(m2,W1.T) + b1)
