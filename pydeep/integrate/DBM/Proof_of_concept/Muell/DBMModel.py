...
# Load Data
data, data_l, valid , valid_l , test, test_l= IO.load_MNIST("../../../../workspacePy/data/mnist.pkl.gz",False)
...
'''
con1 = numx.hstack((Weight_layer.generate_2D_connection_matrix(28, 28, 10, 10, 9, 9, False),
                   Weight_layer.generate_2D_connection_matrix(28, 28, 10, 10, 9, 9, False)))
con1 = numx.hstack((con1,con1))
print con1.shape
con2 = numx.hstack((Weight_layer.generate_2D_connection_matrix(19*2, 19*2, 30, 30, 29, 29, False),
                   Weight_layer.generate_2D_connection_matrix(19*2, 19*2, 30, 30, 29, 29, False)))
con2 = numx.hstack((con2,con2))
print con2.shape

con3 = numx.hstack((Weight_layer.generate_2D_connection_matrix(10, 10, 10, 10, 9, 9, False),
                   Weight_layer.generate_2D_connection_matrix(10, 10, 10, 10, 9, 9, False)))
con3 = None
...
wl3 = Weight_layer(input_dim = O, 
                                 output_dim = P, 
                                 #mask = None,#Convolutional_weight_layer.construct_gauss_filter(3,3,variance = 0.5),
                                 initial_weights = 0.01,
                                 connections = con3,
                                 dtype = numx.float64)
...
wl1 = Convolutional_weight_layer(input_dim = N, 
                                 ...
                                 mask = Convolutional_weight_layer.construct_gauss_filter(3,3,variance = 0.5),
                                 initial_weights = 0.01,
                                 connections = None,#Weight_layer.generate_2D_connection_matrix(28, 28, 10, 10, 9, 9, False), 
                                 dtype = numx.float64)
...
wl2 = Convolutional_weight_layer(input_dim = M, 
                                 ...
                                 mask = Convolutional_weight_layer.construct_gauss_filter(3,3,variance = 0.5),
                                 initial_weights = 0.01,
                                 connections = None,#Weight_layer.generate_2D_connection_matrix(19, 19, 10, 10, 9, 9, False), 
                                 dtype = numx.float64)
...
wl3 = Weight_layer(input_dim = O, 
                   output_dim = P, 
                   initial_weights = 0.01,
                   connections = None,#Weight_layer.generate_2D_connection_matrix(10, 10, 7, 7, 6, 6, False),
                   dtype = numx.float64)

wl3 = Weight_layer(input_dim = O, 
                                 output_dim = P, 
                                 #mask = None,#Convolutional_weight_layer.construct_gauss_filter(3,3,variance = 0.1),
                                 initial_weights = 0.01,
                                 connections = None,#Weight_layer.generate_2D_connection_matrix(19, 19, 10, 10, 9, 9, False), 
                                 dtype = numx.float64)
'''
...
for epoch in range(0,max_epochs) :
    ...
    '''
    logReg = LogisticRegression(C=1, penalty='l2', tol=0.01)
    logReg.fit(a_e, data_l)
    print logReg.score(a_e, data_l)
    x_e = test
    y_e = numx.zeros((10000,v21*v22))+l2.offset
    z_e = numx.zeros((10000,v31*v32))+l3.offset
    a_e = numx.zeros((10000,v41*v42))+l4.offset
    for i in range(k_pos):
        y_e = l2.activation(x_e, z_e)[0]
        a_e = l4.activation(z_e, None)[0]
        z_e = l3.activation(y_e, a_e)[0]
    print logReg.score(a_e, test_l)
    '''
# End time measure
measurer.end()
print
...
VIS.imshow_matrix(VIS.tile_matrix_rows(data[0:100].T, v11,v12, 10,10, border_size = 1,normalized = False), 'data') 
VIS.imshow_matrix(VIS.tile_matrix_rows(x_e[0:100].T, v11,v12, 10,10, border_size = 1,normalized = False), 'sample 1')
VIS.imshow_matrix(VIS.tile_matrix_rows(y_e[0:100].T, v21,v22, 10,10, border_size = 1,normalized = False), 'sample 2')
...
d2 = l2.activation(x_d,numx.zeros((batch_size,v31*v32)))
d2 = l2.sample(d2)
d1 = l1.activation(None,d2)
VIS.imshow_matrix(VIS.tile_matrix_columns(d1[0], v11,v12, 1, batch_size, 1, False),'m 1')
d1 = l1.sample(d1)
d3 = l3.activation(d2, None)
d3 = l3.sample(d3)
d2 = l2.activation(d1,d3)
d2 = l2.sample(d2)
d1 = l1.activation(None,d2)
VIS.imshow_matrix(VIS.tile_matrix_columns(d1[0], v11,v12, 1, batch_size, 1, False),'m 2')
...
d2 = l2.sample(d2)
d1 = l1.activation(None,d2)
VIS.imshow_matrix(VIS.tile_matrix_columns(d1[0], v11,v12, 1, batch_size, 1, False),'m 3')
...
d2 = l2.sample(d2)
d1 = l1.activation(None,d2)
VIS.imshow_matrix(VIS.tile_matrix_columns(d1[0], v11,v12, 1, batch_size, 1, False),'m 4')
'''
VIS.show()
'''
# test 
# Load data
import pydeep.misc.toyproblems as PROBLEM
import pydeep.misc.visualization as VIS

data = PROBLEM.generate_bars_and_stripes_complete(4)
data = numx.vstack((data[0],data,data[5]))

wl1 = Weight_layer(input_dim = 4*4, 
                      output_dim = 5*5, 
                      initial_weights = 'AUTO',
                      connections = None, 
                      dtype = numx.float64)
...
x_m = numx.ones((data.shape[0],4*4))
y_m = numx.ones((data.shape[0],5*5))

#Train model
print 'Training'
print 'Epoch\tRE train_images \tLL train_images '
for epoch in range(1,max_epochs+1) :
    x_d = data
    y_d = l2.activation(x_d, None)[0]
    for i in range(k_m):
        y_m = l2.activation(x_m, None)
        y_m = l2.sample(y_m)
        x_m = l1.activation(None,y_m)
        x_m = l1.sample(x_m)  
    y_m = l2.activation(x_m, None)[0]
    l1.update_offsets(numx.mean(x_d,axis = 0).reshape(1,x_d.shape[1]), shifting)
    l2.update_offsets(numx.mean(y_d,axis = 0).reshape(1,y_d.shape[1]), shifting)
    wl1_grad = wl1.calculate_weight_gradients(x_d, y_d, x_m, y_m, l1.offset, l2.offset)
    wl1.update_weights(epsilon*wl1_grad, None, None)
    grad_b1 =l1.calculate_gradient_b(x_d, x_m, None, l2.offset, None, wl1_grad)
    grad_b2 =l2.calculate_gradient_b(y_d, y_m, l1.offset, None, wl1_grad, None)
    l1.update_biases(epsilon*grad_b1)
    l2.update_biases(epsilon*grad_b2)
    print numx.mean(numx.abs(x_d-x_m))

VIS.imshow_matrix(VIS.tile_matrix_columns(x_d, 4, 4, 32, 1, 1, False),'d')
VIS.imshow_matrix(VIS.tile_matrix_columns(x_m, 4, 4, 32, 1, 1, False),'m')
