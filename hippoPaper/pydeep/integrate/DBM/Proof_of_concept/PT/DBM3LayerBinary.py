...
class Estimator(object):
    ...
    @classmethod
    def _LL_lower_bound_check(cls, model, x, lnZ, conv_thres= 0.00001, max_iter=100000):
        ...
        d3 = numx.copy(model.o3)
        d2 = 0.0
        # While convergence of max number of iterations not reached, 
        # run mean field estimation
        d2_new = Sigmoid.f( id1 + numx.dot(d3-model.o3,model.W2.T) + model.b2)
    ...
    @classmethod
    def LL_lower_bound(cls, model, x, lnZ, conv_thres= 0.00001, max_iter=1000):
        ...
        e1 =  numx.dot(xtemp, model.b1.T)\
            ...
            + numx.sum(numx.dot(xtemp, model.W1) * h1temp ,axis=1).reshape(h1temp.shape[0], 1) \
            + e2
