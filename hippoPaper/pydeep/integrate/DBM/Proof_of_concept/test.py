...
class SoftPlus_layer(Binary_layer):
    ...
    def activation(self, bottom_up_states, top_down_states, bottom_up_offsets, top_down_offsets):
        ...
        if self.output_weight_layer != None:
            pre_act += self.output_weight_layer.propagate_down(top_down_states-top_down_offsets)
        pre_act += self.bias
        return numx.log(1.0+numx.exp(activation)), pre_act
    ...
    def sample(self, activation):
        ...
        '''
        return numx.clip(self.dtype(activation[1] + numx.random.randn(activation[1].shape[0],activation[1].shape[1]))*Sigmoid.f(activation[1]),0.0,10.0)
...
'''
data = numx.vstack((IO.load_CIFAR("../../../../workspacePy/data/CIFAR/data_batch_1",True)[0],
                    IO.load_CIFAR("../../../../workspacePy/data/CIFAR/data_batch_2",True)[0],
                    IO.load_CIFAR("../../../../workspacePy/data/CIFAR/data_batch_3",True)[0],
                    IO.load_CIFAR("../../../../workspacePy/data/CIFAR/data_batch_4",True)[0],
                    IO.load_CIFAR("../../../../workspacePy/data/CIFAR/data_batch_5",True)[0]))
data = preprocessing.remove_rows_means(data)
zca = preprocessing.ZCA(v11 * v12)
zca.train(data)
data = zca.project(data)
...
wl1 = Weight_layer(input_dim = N, 
                   output_dim = M, 
                   initial_weights = 0.01,
                   connections = Weight_layer.generate_2D_connection_matrix(32, 32, 12, 12, 11, 11, False),
                   dtype = numx.float64)
...
for epoch in range(0,max_epochs) :
    ...
    for b in range(0,data.shape[0],batch_size):
        ...
        #    z_d = l3.activation(y_d, None, l2.offset, None)[0]
            #z_d = l3.sample(z_d) 
        #for _ in range(k_pos):
        #    y_d = l2.activation(x_d, numx.roll(z_d,1,0), l1.offset, l3.offset)[0]
        #    z_d = l3.activation(y_d, None, l2.offset, None)[0]
        y_d = l2.activation(x_d, l3.offset, l1.offset, l3.offset)[0]
        z_d = l3.activation(y_d, None, l2.offset, None)[0]
        # negative phase
        ...
        #diff = z_d[1:, :]-z_d[:-1, :]
        grad_b3 =l3.bias_gradient(z_d, z_m)#-numx.sum((diff)**2,axis = 0)/(z_d.shape[0]-1)
        ...
        l1.update_sigma(lr_s1*numxExt.restrict_matrix_norm(grad_s1,max_norm3 ))
    ...
    #print numx.mean(diff**2)
    #print numx.mean(z_d**2)
