...
def rotationSequence(image, width, height, steps):
    ...
    for i in range(1,steps):
        angle = i*360.0/steps
        sample = sci.rotate(image.reshape(width,height),angle)
...
num_seq = 100
train_data = train_data[0:num_seq,:] #+ numx.random.randn(100,28*28)*0.5
steps = 100
data = None
for u in range(0,num_seq):
    samples = rotationSequence(train_data[u],28,28,steps)
    if data == None:
        data = samples.reshape(1,steps,28*28)
    else:
        data = numx.vstack((data,samples.reshape(1,steps,28*28)))
...
l1 = Binary_layer(None, 
                  wl1, 
                  data = data.reshape(num_seq*steps,784), 
                  initial_bias = 'AUTO',
                  initial_offsets = 'AUTO',
                  dtype = numx.float64)
...
for epoch in range(0,max_epochs) :
    for b in range(0,data.shape[0]):
        ...
        grad_b1 =l1.calculate_gradient_b(x_d, x_m)
        grad_b2 =l2.calculate_gradient_b(y_d, y_m)#+0.1*numx.mean(numx.eye(100,100)-numx.dot(y_d,y_d.T),axis = 0)#-numx.mean((y_d[1:, :]-y_d[:-1, :])**2,axis = 0)
        grad_b3 =l3.calculate_gradient_b(z_d, z_m)+0.1*numx.mean(numx.eye(v31*v32,v31*v32)-numx.dot(z_d.T,z_d),axis = 0)#-0.3*numx.mean(numx.eye(4,4)-numx.dot(z_d[1:, :],z_d[:-1, :].T),axis = 0)#-0.3*numx.mean((z_d[1:, :]-z_d[:-1, :])**2,axis = 0)
        print numx.mean(numx.abs(numx.eye(v31*v32,v31*v32)-numx.dot(z_d.T,z_d)))
        # Update Model
...
x_m = x_d
z_m = l3.offset
VIS.imshow_matrix(VIS.tile_matrix_columns(x_m, v11, v12, 1, batch_size, 1, False),'d')
for i in range(4):
    y_m = l2.activation(x_m, z_m, l1.offset, l3.offset)
    y_m = l2.sample(y_m)
    x_m = l1.activation(None,y_m, None, l2.offset)
    ...
    z_m = l3.activation(y_m, None, l2.offset, None)
    z_m = l3.sample(z_m) 
VIS.imshow_matrix(VIS.tile_matrix_rows(z_d, 1,100, v31*v32,1, border_size = 0,normalized = False), 'samples 1')
